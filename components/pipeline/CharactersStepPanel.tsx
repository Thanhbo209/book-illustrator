...
export function CharactersStepPanel({ project }: { project: ProjectDetail }) {
  ...
  async function submit() {
    ...
    try {
      const response = await fetch(`/api/projects/${project.id}/steps/characters`, {
        method: "POST",
      });
  ...
  if (project.currentStep !== "CHARACTERS") return null;

  if (isSubmitting || isRunning) {
    return (
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <span className="size-2 animate-pulse rounded-full bg-primary" />
        {STEP_LABELS.CHARACTERS}
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Button onClick={submit} className="self-start">
        {isStale ? "Recover step" : isFailed ? "Retry" : "Generate characters"}
      </Button>
    </div>
  );
}
